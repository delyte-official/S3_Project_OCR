...
double Random()
{
	return ((double)rand() / ((double)RAND_MAX /2) - 1);
     // Box-Muller transform for generating standard normal values
    /*double u1 = ((double)rand() / RAND_MAX);
    double u2 = ((double)rand() / RAND_MAX);
    double z0 = sqrt(-2.0 * log(u1)) * cos(2.0 * M_PI * u2);

    // He initialization: scaling by sqrt(2 / n), where n is the input size
    int n = 28 * 28; // Input layer size
    double stddev = sqrt(2.0 / n);

    return z0 * stddev; // Return He-initialized random value*/
}
